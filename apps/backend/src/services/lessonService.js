...
exports.createLesson = async (userId, { title, description, content, status, tags }) => {
  const result = await pool.query(
    `INSERT INTO lessons (title, description, content, status, user_id, tags)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, title, description, content, status, tags, created_at`,
...
exports.getLesson = async (lessonId) => {
  const result = await pool.query(
    `SELECT id, title, description, content, status, user_id, tags, created_at
     FROM lessons
     WHERE id = $1`,
    [lessonId]
  );
...
exports.updateLesson = async (userId, lessonId, payload) => {
  const check = await pool.query(
    "SELECT user_id FROM lessons WHERE id = $1",
    [lessonId]
  );

  if (check.rows.length === 0) throw new Error("Lesson not found");
